...
* 
* this is a wrapper class around the php code from:
* http://sourceforge.net/projects/phputf8
* 
* wich is based on the perl codes from:
* http://search.cpan.org/~sburke/Text-Unidecode-0.04/
* http://interglacial.com/~sburke/
*/

class tx_naworkuri_utf8_to_ascii {
	...
	*/
	...
	var $utf8_to_ascii_path  = '';
	var $utf8_to_ascii_cache = array();
	
	function __construct(){
		$this->utf8_to_ascii_cache = array();
		$this->utf8_to_ascii_path  = t3lib_extMgm::extPath('nawork_uri').'/lib/utf8_to_ascii/db';
	} 
	...
	//     static $UTF8_TO_ASCII = array();
	    ...
	    while ( $i < $len ) {
	        ...
	        if ( !array_key_exists($bank, $this->utf8_to_ascii_cache) ) {
	            
	            $bankfile = $this->utf8_to_ascii_path. '/'. sprintf("x%02x",$bank).'.php';
	          	
	            if ( file_exists($bankfile) ) {
	                # Load the appropriate database
	                if ( !include_once($bankfile) ) {
	                    ob_end_clean();
